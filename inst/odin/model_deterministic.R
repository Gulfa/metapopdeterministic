...
contact_change[,] <- if(kernel[i,j] > 15) 1 else ( if(spont_behav_mode==1) exp(kernel[i,j])/(1 + exp(kernel[i,j])) else (spont_behav_change_params[7] - log(exp(a*C[i,j]) + exp(a*W))/a + log(exp(a*C[i,j])+1)/a)/spont_behav_change_params[7])
